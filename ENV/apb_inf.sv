...
interface apb_inf(input logic pclk);
    ...
    assert property(setup_to_access_check)
      ...
      $error("APB ASSERTION FAILED: SETUP did not transition to ACCESS");
    ...
    assert property(p_addr_stable_wait)
      ...
      $error("APB ASSERTION FAILED : PADDR changed during wait state");
    ...
    assert property(p_write_stable_wait)
      ...
      $error("APB ASSERTION FAILED : PWRITE changed during wait state");
    ...
    assert property(p_pwdata_stable_wait)
      ...
      $error("APB ASSERTION FAILED : PWDATA changed during write wait state");
    ...
    assert property(p_psel_hold_wait)
      ...
      $error("APB ASSERTION FAILED : PSEL dropped during wait state");
    ...
    assert property(p_penable_hold_wait)
      ...
      $error("APB ASSERTION FAILED : PENABLE dropped during wait state");
    ...
    ap_no_unkown_control: assert property(p_no_unknown_control)
    else
      $error("APB ASSERTION FAILED : Unknown value detected on control signals");
    ...
    assert property(p_pslverr_valid)
      ...
      $error("APB ASSERTION FAILED : PSLVERR asserted outside valid transfer completion");
    ...
    assert property(p_reset_behavior)
      ...
      $error("APB ASSERTION FAILED : Invalid signal values during reset");
    ...
    assert property(p_access_after_setup)
      ...
      $error("APB ASSERTION FAILED : ACCESS occurred without prior SETUP");
    ...
    assert property(p_idle_state_valid)
      ...
      $error("APB ASSERTION FAILED : Invalid IDLE state");
